...
#'
#' @return  \code{pnwd} gives the distribution function,
...
pnwd <- function(x, beta = 1, alpha = 2) {
  if (any(alpha <= 0) || any(beta <= 0)) stop(message = "incompatible arguments.")
  if (any(x < 0)) stop(message = "[Warning] 0 < x ")
  if (is.vector(x)) {
    F0 <- stats::pweibull(x, shape = beta[1], scale = alpha[1])
  } else {
    if (length(alpha) < 2 || length(beta) < 2) {
      stop(message = "incompatible arguments.")
    } else {
      F0 <- matrix(data = NA, nrow = nrow(x), ncol = 2)
      F0[, 1] <- stats::pweibull(x[, 1], shape = beta[1], scale = alpha[1])
      F0[, 2] <- stats::pweibull(x[, 2], shape = beta[2], scale = alpha[2])
    }
  }
  return(F0)
}
...
#' @export
dnwd <- function(x, beta = 1, alpha = 2) {
  if (any(alpha <= 0) || any(beta <= 0)) stop(message = "incompatible arguments.")
  if (any(x < 0)) stop(message = "[Warning] 0 < x ")
  if (is.vector(x)) {
...
#' @export
qnwd <- function(q, beta = 1, alpha = 2) {
...
#' @export
rnwd <- function(n, beta = 1, alpha = 2) {
  if (any(alpha <= 0) || any(beta <= 0)) stop(message = "incompatible arguments.")
  if (length(alpha) < 2 || length(beta) < 2) {
    u <- runif(n)
    X <- qnwd(u, alpha, beta)
